...
export default function AddToFavsButton({active}: IAddToFavsButtonProps):JSX.Element {
  const {addToFavs, removeFromFavs} = useArticleContext()
  const articleTest:cartArticle = {
    id: 10,
    titulo: 'string',
    descripcion: 'string',
    categoria: 'string',
  ...
  const handleAddToFavs = ():void => {
    addToFavs(articleTest)
  }

  const handleRemoveFromFavs = ():void => {
    removeFromFavs(articleTest)
  }
  ...
  return (
    <button
      ...
      >
      <svg
        className={`w-6 h-6 ${active ? 'fill-primary': 'fill-white'} transition-all hover:scale-110 hover:fill-primary/60`}
        viewBox="0 0 256 256"
        id="Flat"
